...
export function NotificationPanel({ open, onClose }: NotificationPanelProps) {
  ...
  const panelOpacity = open && !isAnimating ? 'opacity-100' : 'opacity-0';
  const panelScale = open && !isAnimating ? 'scale-100' : 'scale-95';

  return (
    <>
      <div
        className={`fixed inset-0 z-notification bg-black/30 backdrop-blur-sm transition-all duration-200 md:hidden ${
      ...
      <div
        ref={panelRef}
        className={`fixed right-4 left-4 top-16 z-[160] mx-auto w-auto overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl transition-all duration-200 ease-out dark:border-slate-700 dark:bg-slate-900 md:absolute md:inset-x-auto md:right-0 md:top-full md:mx-0 md:mt-2 md:w-80 ${
          open ? panelOpacity : 'pointer-events-none opacity-0'
        } ${open ? panelScale : 'scale-95'}`}
        role="dialog"
